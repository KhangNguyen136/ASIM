...
class editExOrInVC: UIViewController,selectCategoryDelegate,selectAccountDelegate {
    ...
    @IBAction func saveRecord(_ sender: Any) {
        ...
        if type == 0
        {
            ...
            if srcAccount != temp?.srcAccount || _amount != temp?.amount || detailCategory != temp?.detailCategory || category != temp?.category || dateTime.date != temp?.date || descript.text != temp?.descript || personTF.text != temp?.payee || locationTF.text != temp?.location || eventTF.text != temp?.event
            {
                ...
                //undo old-repayment
                if temp?.borrowRecord != nil
                {
                    temp?.borrowRecord?.borrow?.undoRepay(_amount: temp!.amount)
                }
        ...
        else
        {
            ...
            if srcAccount != temp?.srcAccount || _amount != temp?.amount || detailCategory != temp?.category || descript.text != temp?.descript || personTF.text != temp?.payer || locationTF.text != temp?.location || eventTF.text != temp?.event
            {
                ...
                temp?.undoTransaction()
                if temp?.lendRecord != nil{
                    temp?.lendRecord?.lend?.undoCollect(_amount: temp!.amount)
                }
...
//            }
    ...
    @IBAction func clickDelete(_ sender: Any) {
        try! realm.write{
            if record?.type == 0
            {
                if record?.expense?.borrowRecord != nil
                {
                    record?.expense?.borrowRecord?.borrow?.undoRepay(_amount: (record?.expense!.amount)!)
                }
            ...
            {
                if record?.income?.lendRecord != nil
                {
                    record?.income?.lendRecord?.lend?.undoCollect(_amount: (record?.income!.amount)!)
                }
            ...
            realm.delete(record!)
            print("Deleted a income or expense!")
